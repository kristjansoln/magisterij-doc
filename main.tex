...
\chapter{Sorodna dela} \label{sct:sorodna-dela}
    ...
    \section{Razpoznavanje hrane iz slikovnih vzorcev} \label{sct:razpoznavanje-jedi-na-slikah}
        ...
        % Weakly supervised ingredient segmentation/detection
            ...
            Sorodno so Zhu~\etal~\cite{src:zhu2023new} predstavili arhitekturo AttNet. Posamezne sestavine so razdelili v štiri nivoje kategorij, AttNet pa je za dano sliko poskušal napovedati vse štiri nivoje. Tako kot v prejšnjem pristopu~\cite{src:vlachopoulou2023food} so tudi tu za vsako sestavino naučili lasten model. Kasneje so v~\cite{src:zhu2023new} uporabili značilke iz konvolucijskih plasti v mreži in s filtri ter rojenjem izrazili približek segmentacijske maske. Model se ni neposredno učil ločevanja meje med sestavinami, le ločevanje globalnega videza posamezne sestavine. Ker so konvolucijske značilke lokalne, se je ista aktivacija pojavila točno tam, kjer so bili slikovni elementi te sestavine, kar je omogočilo kasnejšo rekonstrukcijo maske. S tako pridobljenimi segmentacijskimi maskami so poročali do 65{,}3~\% mIoU na FoodSeg103~\cite{src:wu2021large}. Njihovi rezultati, čeprav zelo dobri, niso bili neposredno primerljivi z drugimi metodami, saj so avtorji omenjeno zbirko modificirali - ozadje so zamenjali z modro barvo. %Slabosti te metode?
    ...
    \section{Izvajanje na robnih napravah} \label{sct:izvajanje-na-robnih-napravah}
        ...
        % Early efforts
        Za področje izvajanja modelov na robnih napravah se pogosto uporablja termin TinyML~\cite{src:abadade2023comprehensive}. Med začetke teh pristopov sodijo Kawano~~\etal~\cite{src:kawano2014foodcam} z modelom FoodCam-256, ki je bil pripravljen za izvajanje na mobilnih napravah, a je uporabljal tradicionalne pristope za klasifikacijo. Med pomembnejše pristope na tem področju sodita še SqueezeNet~\cite{src:iandola2016squeezenet} ter MobileNetV1~\cite{src:howard2017mobilenets}, ki sta v primerjavi z AlexNet~\cite{src:krizhevsky2012imagenet} ohranila oz. izboljšata natančnost, hkrati pa močno zmanjšala število parametrov.
...
\chapter{Teoretično ozadje} \label{sct:teoreticno-ozadje}
    ...
    V tem poglavju predstavimo teoretično ozadje pristopov, ki jih obravnavamo v tem delu. V odsekih \ref{sct:teorija-klasifikacija} in \ref{sct:teorija-segmentacija} podamo osnovne pojme klasifikacije in segmentacije na podlagi slikovnih vzorcev, najprej splošno, nato pa še v kontekstu razpoznavanja jedi.
V odseku \ref{sct:teorija-robnenaprave} obravnavamo pristope izvajanje modelov globokega učenja na robnih napravah, naredimo pregled najpogostejših pristopov k optimizaciji ter predstavimo obstoječe programske rešitve in razvojna okolja, namenjena razvoju in uporabi takšnih sistemov.
    ...
    \section{Razpoznavanje na podlagi slikovnih vzorcev} 
        ...
        \subsection{Klasifikacija jedi}\label{sct:teorija-klasifikacija}
            ...
            % TODO: Klasicne metode
            % Razširi. Povej recimo kake pristope za luščenje značilk lahk mamo, kake razvrščevalnike lahk uporabmo ipd.
            Značilke v tradicionalnih klasifikacijskih pristopih tipično luščimo ročno, nato pa jih uporabimo v razvrščevalniku~\cite{src:min2019survey}.

            % TODO: Moderne metode
            Pristopi, ki uporabljajo globoko učenje, nam omogočijo precej boljše rezultate.
            Alternativno tradicionalnim pristopom 

            % Metode s komponentami

            % TODO:Sibko nadzorovane
            Nekateri izmed pristopov temeljijo na šibkem učenju. Zanj je značilno, da ...
            Tak pristop omogoča, da se izognemo potrebi po segmentacijskih podatkovnih zbirkah, saj za učenje zadoščajo že anotacije na ravni sliko o prisotnosti sestavin. Model med učenjem nato na podlagi ... sklepa ...

        \subsection{Segmentacija jedi} \label{sct:teorija-segmentacija}

            % TODO: kaj je segmentacija, kako se to v splosnem dela, instancna vs semanticna segmentacija

            % TODO: Tradicitonalni pristopi

            % TODO: Moderni pristopi

            % TODO: Zero shot metode
...
\chapter{Metodologija} \label{sct:metodologija}
    ...
    \section{FOMO} \label{sct:fomo}
        ...
        FOMO (Faster Objects, More Objects)~\cite{src:announcingfomo, src:fomodocs} je detekcijski model, namenjen realnočasovnemu izvajanju na mikrokrmilnikih. Na voljo je na platformi \mbox{Edge Impulse}~\cite{src:fomodocs}. Zanj je značilna nizka poraba pomnilnika in kratek čas inference - avtorji v demonstraciji prikažejo izvajanje na Cortex M7 jedru razvojnega kompleta Arduino Nicla Vision, kjer dosežejo obdelavo do 28 sličic na sekundo ob porabi zgolj 245 kB pomnilnika~\cite{src:fomodocs}.
