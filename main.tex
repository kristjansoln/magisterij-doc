...
\chapter{Uvod} \label{sct:uvod}
    ...
    % %- Hrana kot vsakdan in kultura
    % Priprava in uživanje hrane sta vsakodnevni dejavnosti ljudi in velik del kulture.
    % %- Zgodovinski fokus
    % Tekom zgodovine je bila glavna skrb povezana s hrano predvsem s količino ter da hrano naredi lažje prebavljivo in varno za uživanje.
    % %- Današnji fokus v izobilju - industrijska revolucija, napredki na področju pasterizacije, masovne proizvodnje in razumevanja kemijskih procesov med pripravo
    % To se močno spremeni tekom zgodovine, z razlogi kot je industrijska revolucija in s tem povezan razvoj masovne proizvodnje, napredki na področju pasterizacije in razumevanje kemijskih procesov med pripravo hrane.
    % Danes so tako glavni poudarki pogosteje na okusu, konsistenci, ponovljivosti in preprostosti priprave \cite{src:mcgee2004mcgee}.
    % %- Zdrava priprava kot pragmatična izbira
    % V tem okviru se vse pogosteje pojavlja tudi vidik zdrave priprave: izbira tehnik, ki ohranijo hranila in zmanjšajo nepotrebne dodatke. % To je narobe
    % %- Most do tehnologije

    % - priprava hrane na splošno?
    ...
    %- zakaj je pomembna - mogoče s tem naredimo hrano lažje prebavljivo in bolj varno?
        Toplotna obdelava živil praviloma poveča njihovo prebavljivost in s tem olajša izrabo energije ter nekaterih hranil. Posledično je kuhanje našim zgodnjim prednikom omogočilo izkoriščanje širšega nabora prehranskih virov in iz njih pridobiti več hranilne vrednosti \cite{src:myhrvold_cooking_2025}.
    ...
    % Povezava z zdravjem in okoljem
        Prehrana tako močno vpliva na zdravje, hkrati pa je pomemben faktor za zmanjšanje vpliva na okolje \cite{src:eat2019food}.
    ...
    Nove tehnologije – zlasti računalniški vid na robnih napravah – lahko proces priprave hrane standardizirajo in olajšajo: zmanjšajo napake, skrajšajo čas ter razbremenijo uporabnika pri načrtovanju in izvedbi korakov. Že zgodnji poskusi nadzora peke so vključevali optične sisteme z obdelavo slik v pečici za spremljanje vizualnih značilnosti (barvni ton, teksturo, obliko) in s tem postavili temelje za samodejno vodenje procesa \cite{src:paquet2012monitoring}. V zadnjih letih so se metode globokega učenja uveljavile pri razpoznavi jedi in ocenjevanju količin, kar poganja razcvet aplikacij, ki pripravo deloma avtomatizirajo ali vsaj poenostavijo \cite{src:SEDEJ_2023, src:Nikodinovska_2024}. Razpoznava jedi je zato ključen gradnik: sistemu omogoči, da v realnem času razume, katera jed je prisotna, in na tej osnovi prilagodi naslednje korake ter pripravo naredi bolj zanesljivo in enostavno.
    
    % Zakaj je razpoznavanje sestavin hrane pomembno (bolj univerzalen pristop, ne bi rabli za dodajanje nove jedi nujno delat novega dataseta. Temu smo rekli razpoznavanje kompleksnih jedi).
    Identifikacija celotnih jedi je v literaturi precej pogosto obravnavana naloga \cite{src:vlachopoulou2023food, src:mohanty2022food, src:bolanos2017food, src:hassannejad2016food, src:martinel2018wide, src:tan2019efficientnet} na raznih podatkovnih zbirkah \cite{src:bossard2014food, src:matsuda2012recognition, src:kawano2014automatic, src:chen2016deep}, vendar te tipično predpostavljajo zaprt, vnaprej določen seznam jedi.
    Nasprotno pa razpoznavanje sestavin omogoči, da jed sklepamo iz zaznanih komponent: nove jedi lahko dodajamo kot kombinacije sestavin (npr. z enostavno preslikavo med kombinacijami in jedmi), brez širjenja podatkovne zbirke ali ponovnega učenja modela. Tak pristop bolje prenaša visoko vizualno varianco in omogoča razpoznavo kompleksnih jedi z več komponentami, kjer se izgled med primeri močno razlikuje. Hkrati se labelni prostor bistveno zmanjša, saj lahko iz manjšega nabora sestavin tvorimo veliko število jedi \cite{src:bolanos2017food}. Zato je pristop, utemeljen na sestavinah, bolj univerzalen  od neposredne klasifikacije jedi, posebej tam, kjer moramo sistem enostavno širiti in prilagajati.

    % Povej da se novejši pristopi tipično lotijo segmentacije sestavin hrane
    Novejši pristopi se razpoznavanja sestavin načeloma lotijo kot segmentacijski problem \cite{src:wu2021large, src:lan2023foodsam, src:bolanos2016simultaneous, src:pfisterer2019segmentation, src:poply2021refined}, pogosto na zbirkah, kot je FoodSeg103 \cite{src:wu2021large}. Segmentacija (semantična ali instančna) deluje na podlagi pikselno natančnih mask in omogoča celo ocenjevanja količin ter hranilnih vrednosti, vendar so pripadajoče arhitekture računsko in pomnilniško zahtevne, kar včasih oteži realno uporabo. Deloma se lahko temu izognemo z izvajanjem v oblaku, a to prinese nove zahteve glede latence, stroškov, povezljivosti naprav in zasebnosti; trend v praksi je zato optimizacija in izvajanje neposredno na ciljnih napravah \cite{src:abadade2023comprehensive}. Za robne platforme z omejenimi viri to pomeni iskanje kompromisov med natančnostjo in kompleksnostjo ter premislek, ali je polna segmentacija res nujna ali jo lahko nadomestimo z lažjimi formulacijami naloge.
    
    % Kaj je robna naprava, povej o pomembnosti edge devicov, FOMO, argumentiraj in na hitro opiši.
    Robna naprava (angl. \textit{edge device}) je lokalna strojna enota na robu omrežja - od naprav Raspberry Pi in pametnih telefonov vse do mikrokrmilnikov - ki podatke praviloma zajame in vsaj del obdelave izvede lokalno; zaradi strogih omejitev pomnilnika, računske moči in porabe energije uporaba globokih modelov zahteva ciljno optimizacijo, kar je danes zelo aktivno področje raziskav in razvoja \cite{src:abadade2023comprehensive, src:moosmann2024ultra_tinyissimoyolo, src:boyle2024dsort, src:lin2021memory, src:hymel2022edgeimpulse}.
    V tem delu uporabimo model FOMO (angl. Faster Objects, More Objects) platforme Edge Impulse \cite{src:announcingfomo}, detekcijski pristop za mikrokrmilnike, ki se izogne klasični strukturi detektorjev in na izhodu ustvari grobo segmentacijsko masko; to se običajno naknadno obdela (npr. z združevanjem/rojenjem), da dobimo lokacije objektov. Ker nas v kontekstu tega dela zanima prisotnost sestavin, postopek priredimo tako, da informacijo o lokalizaciji opustimo in  gledamo zgolj zadostno prisotnost sestavin na maski. Model je zelo učinkovit na omejenih virih, tekom dela pa se pokažejo tudi slabosti majhnega receptivnega polja in omejene kapacitete modela, kar omeji končno uspešnost pristopa \cite{src:boyle2024dsort}.

    % Omejitev strojne opreme in izbor platforme STM32
    V nadaljevanju se v skladu z željeno izvedbo na robnih napravah v grobem omejimo tudi na strojno opremo: kot platformo izberemo mikrokrmilnike STM32 s tipičnimi omejitvami pomnilnika in shranjevalnega prostora do nekaj MB. Čeprav so te zahteve stroge, so se v preteklosti že izkazale za izvedljive \cite{src:Nikodinovska_2024}. Pristop dodatno podpira orodje X-CUBE-AI za izvajanje modelov \cite{src:stxcubeai-docs} ter nabor referenčnih modelov, dokumentiranih in pripravljenih za izvajanje na izbrani platformi \cite{src:stm32modelzoo}.
    ...
    Namen te naloge je torej eksperimentalno preveriti izvedljivost razpoznave sestavin hrane na izbranih robnih napravah z izbranim pristopom FOMO, pri čemer uporabimo podatkovno zbirko FoodSeg103. Iz ugotovitev želimo izpeljati praktične smernice za praktično in industrijsko rabo, kjer so ključne omejitve strojna oprema in robustnost v realnih pogojih. Problema se lotimo kot razpoznave sestavin hrane, ki predstavlja osnovo za nadaljnje delo, z vizijo kot razpoznavo kompleksnih jedi na osnovi sestavin. Uporabimo alternativen pristop razpoznave sestavin namesto klasičnih segmentacijskih modelov, da je izvedba na ciljni strojni opremi lažja.
    Izbran pristop želimo ovrednotiti na FoodSeg103 in analizirati njegove omejitve. Prikazati želimo vpliv priprave modela za robno izvajanje, kar zajema predvsem vpliv kvantizacije na velikost in uspešnost modela. Preučiti želimo tudi poenostavitve podatkovne zbirke, da lahko problem zastavimo drugače (npr. kot detekcijski problem). Na koncu na podlagi ugotovitev oblikujemo jasne smernice in predloge za nadaljnje delo.

    % Omejitve in izzivi
    Zastavljen problem je zahteven zaradi odprtosti okolja, tipične neenakomerne porazdelitve razredov v podatkovnih zbirkah sestavin, prekrivanj objektov na sliki, visoke znotrajrazredne variabilnosti ter nizke medrazredne variabilnosti \cite{src:wu2021large, src:bossard2014food}. Dodatno ga otežujejo omejitve strojne opreme in izbrane platforme, kot so velikost modela ter uporabljen pomnilnik, ki so pogosto omejeni na nekaj megabajtov ali celo kilobajtov. Dodatno omejitev predstavlja podpora izvajalnega okolja, saj izvajalna okolja za robne naprave načeloma podpirajo precej ožji nabor operacij.
...
\chapter{Sorodna dela} \label{sct:sorodna-dela}
    ...
    \section{Izvajanje na robnih napravah}
        ...
        % Segemntacija?
        % Deeplabv3 iz model zoo in YOLO V8 iz stm32 model zoo? Noben prav zares ne paše na naš hardware...
...
\chapter{Metodologija} \label{sct:metodologija}
    ...
    V tem poglavju podrobneje predstavimo arhitekturo in lastnosti modela FOMO ter postopke predobdelave in naknadne obdelave podatkov. S teoretičnega vidika opišemo tudi pristope YOLOv11-seg in družino SAM, s katerimi želimo preučiti možnosti poenostavitve problema in prilagoditve podatkovne zbirke, da lahko nalogo po potrebi zastavimo drugače, na primer kot detekcijski problem.
