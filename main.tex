...
\chapter{Uvod} \label{sct:uvod}
    ...
    Nove tehnologije – zlasti računalniški vid na robnih napravah – lahko proces priprave hrane standardizirajo in olajšajo: zmanjšajo napake, skrajšajo čas ter razbremenijo uporabnika pri načrtovanju in izvedbi korakov. Že zgodnji poskusi nadzora peke so vključevali optične sisteme z obdelavo slik v pečici za spremljanje vizualnih značilnosti (barvni ton, teksturo, obliko) in s tem postavili temelje za samodejno vodenje procesa~\cite{src:paquet2012monitoring}.V zadnjih letih so se metode globokega učenja uveljavile na področju razpoznave jedi in ocenjevanja količin, kar je spodbudilo razvoj aplikacij, ki proces priprave deloma avtomatizirajo ali vsaj poenostavijo~\cite{src:SEDEJ_2023, src:Nikodinovska_2024}.
    Razpoznava jedi je zato ključen gradnik, s katerim sistemom omogočimo, da v realnem času določijo, katera jed je prisotna, na tej osnovi prilagodijo naslednje korake in tako naredijo pripravo bolj zanesljivo in enostavno.
    ...
    Identifikacija jedi je v literaturi precej pogosto obravnavana naloga~\cite{src:vlachopoulou2023food, src:mohanty2022food, src:bolanos2017food, src:hassannejad2016food, src:martinel2018wide, src:tan2019efficientnet} na raznih podatkovnih zbirkah~\cite{src:bossard2014food, src:matsuda2012recognition, src:kawano2014automatic, src:chen2016deep}. Pri tem večina pristopov tipično predpostavlja zaprt, vnaprej določen seznam jedi. Pri tem večina pristopov tipično predpostavlja zaprt, vnaprej določen seznam jedi. Takšna predpostavka pogosto vodi v slabšo fleksibilnost sistema, saj brez dodatnega učenja težko spreminjamo izbran nabor jedi.

    Nasprotno pa razpoznavanje sestavin omogoči, da tip jedi določimo na podlagi zaznanih komponent: nove jedi lahko dodajamo kot kombinacije sestavin (npr. z enostavno preslikavo med kombinacijami in jedmi), brez širjenja podatkovne zbirke ali ponovnega učenja modela.
    ...
    Raziskovalci se danes razpoznavanja sestavin lotijo kot segmentacijski problem~\cite{src:wu2021large, src:lan2023foodsam, src:bolanos2016simultaneous, src:pfisterer2019segmentation, src:poply2021refined}. Pri tem pogosto uporabijo zbirke, kot je FoodSeg103~\cite{src:wu2021large}.
    % TODO: Pikselno natančne maske - prevod
    Segmentacija (semantična ali instančna) deluje na podlagi pikselno natančnih mask, s čimer lahko dosežemo tudi ocenjevanje količin ter hranilnih vrednosti. Pripadajoče arhitekture so načeloma računsko in pomnilniško zahtevne, zato jih težje uporabljamo na realnih primerih.
    Z izvajanjem v oblaku se lahko izognemo navedenim težavam, vendar s tem vnesemo dodatne probleme v povezavi z latenco, stroški, povezljivostjo naprav ter zasebnostjo. Trend v praksi je zato optimizacija in izvajanje neposredno na robnih napravah~\cite{src:abadade2023comprehensive}.
    Za robne naprave z omejeno zmogljivostjo to pomeni iskanje kompromisov med natančnostjo in kompleksnostjo, dobro pa je premisliti tudi, če lahko polno segmentacijo nadomestimo s preprostejšim pristopom.
    
    % Kaj je robna naprava, povej o pomembnosti edge devicov, FOMO, argumentiraj in na hitro opiši.
    Robna naprava (angl. \textit{edge device}) je lokalna strojna enota na robu omrežja~-~od naprav Raspberry Pi in pametnih telefonov vse do mikrokrmilnikov - ki podatke praviloma zajame in vsaj del obdelave izvede lokalno~\cite{src:abadade2023comprehensive, src:moosmann2024ultra_tinyissimoyolo, src:boyle2024dsort, src:lin2021memory, src:hymel2022edgeimpulse}.
...
\chapter{Sorodna dela} \label{sct:sorodna-dela}
    ...
    \section{Razpoznavanje jedi na podlagi slik}
        ...
        \subsection{Klasifikacija jedi}
            ...
            Hassannejad~\textit{et al.}~\cite{src:hassannejad2016food} so model Inception~V3~\cite{src:szegedy2016rethinking} doučili na zbirkah UEC-Food100, UEC-Food256 in Food-101. Dosegli so natančnosti 81{,}5~\%, 76{,}2~\% ter 88{,}3~\%. Čeprav so dobre rezultate dosegli s precej manj parametri kot konkurenca, njihov pristop še vedno ni bil ustrezen za izvajanje na robnih napravah.
            % Matrinel 2018 - WISeR
            ...
            ter dosegli do 80{,}1~\% po F1 klasifikacijski metriki. Preverili so tudi sposobnost generalizacije modela na nove slike jedi, kjer so na modificirani zbirki Recipes5k % TODO CITIRAJ
    ...
    \section{Izvajanje na robnih napravah}
        ...
        % Early efforts
        Za področje izvajanja modelov na robnih napravah se pogosto uporablja termin TinyML~\cite{src:abadade2023comprehensive}. Med začetke teh pristopov sodijo Kawano~\textit{et al.}~\cite{src:kawano2014foodcam} z modelom FoodCam-256, pripravljenim za izvajanje na mobilnih napravah, a uporabljajo tradicionalne pristope za klasifikacijo. Med pomembnejše pristope na tem področju sodita še SqueezeNet~\cite{src:iandola2016squeezenet} ter MobileNetV1~\cite{src:howard2017mobilenets}, ki v primerjavi z AlexNet~\cite{src:krizhevsky2012imagenet} ohranita oz. izboljšata natančnost, hkrati pa močno zmanjšata število parametrov.
        ...
        % Klasifikacija
        V STM32 Model Zoo~\cite{src:stm32modelzoo} na zbirki \mbox{Food-101} navajajo do 77{,}1~\% natančnost z MobileNetV1 ter do 80{,}38~\% natančnost z MobileNetV2. V realnosti je natančnost tipično nižja, saj je potrebno zmanjšati velikost modela glede na izbrano strojno opremo.
...
\chapter{Metodologija} \label{sct:metodologija}
    ...
    \section{FOMO} \label{sct:fomo}
        ...
        FOMO (Faster Objects, More Objects) je detekcijski model, namenjen realnočasovnemu izvajanju na mikrokrmilnikih. Na voljo je na platformi \mbox{Edge Impulse}~\cite{src:fomodocs, src:announcingfomo}. Zanj je značilna nizka poraba pomnilnika in kratek čas inference - avtorji v demonstraciji prikažejo izvajanje na Cortex M7 jedru razvojnega kompleta Arduino Nicla Vision, kjer dosežejo obdelavo do 28 sličic na sekundo ob porabi zgolj 245 kB pomnilnika~\cite{src:fomodocs}. 
    ...
    \section{YOLOv11-seg} \label{sct:yolov11n-seg}
        ...
        Prvo različico so predstavili Redmon~\textit{et al.} leta 2015~\cite{src:redmon2016you}, kjer so detekcijo objektov formulirali kot regresijski problem in se tako izognili tradicionalnemu dvostopenjskemu pristopu. V naslednjih generacijah so sledile številne izboljšave, med drugim večskalen pristop \mbox{(angl. \textit{Multi-scale Detection})~\cite{src:redmon2018yolov3}}, instančna segmentacija~\cite{src:yolov5}, sledenje objektom~\cite{src:yolov8} in ocena ključnih točk (angl. \textit{Keypoint Estimation})~\cite{src:wang2024yolov9}. YOLOv11 je najnovejša iteracija serije, ki jo razvija Ultralytics~\cite{src:ultralyticsyolov11, src:khanam2024yolov11, src:yolov11}.
    ...
    \section{Pretvorba semantično-segmentacijskih mask v okvirje} \label{sct:sam}
        ...
        Zato smo skušali do kakovostnejših rezultatov priti še po alternativni poti: z \emph{zero-shot} segmentacijo objektov s pomočjo modelov \mbox{Segment Anything (SAM)~\cite{src:kirillov2023segment}}. Izziv teh modelov je, da generirajo razredno neodvisne maske, ki jim je treba naknadno pripisati kategorije; dodatno je evalvacija zahtevna, saj težko objektivno kvantificiramo kakovost segmentacije z enotno numerično metriko.
    ...
    \subsection{Naivna pretvorba mask v okvirje} \label{sct:naivnapretvorba}
        ...
        Postopek je preprost in hiter, vendar ob stiku ali prekrivanju dveh instanc istega razreda komponente zlitje obravnava kot en sam objekt ter vrne en sam (prevelik) okvir.
