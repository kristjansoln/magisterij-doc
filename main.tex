...
\chapter{Sorodna dela} \label{sct:sorodna-dela}
    ...
    \section{Razpoznavanje hrane iz slikovnih vzorcev} \label{sct:razpoznavanje-jedi-na-slikah}
        ...
        Zgodnji pristopi k razpoznavanju jedi na podlagi slik so temeljili na ročno izluščenih značilnicah, na primer značilnicah HOG (histogram orientiranih gradientov)~\cite{src:dalal2005hog}, SIFT (angl. \emph{Scale-Invariant Feature Transform})~\cite{src:lowe2004sift} in barvnih histogramih~\cite{src:min2019survey}. V nekaterih zgodnjih delih~\cite{src:wang2003assessment, src:segnini1999low} so avtorji povprečili svetlosti slikovnih elementov na sivinskih slikah ter na tej osnovi določali lastnosti sira med peko in ocenjevali kakovost čipsa.
        V novejših pristopih avtorji pogosto uporabljajo metode globokega učenja, s katerimi dosegajo boljše rezultate~\cite{src:min2019survey, src:kagaya2014food, src:krizhevsky2012imagenet, src:hassannejad2016food, src:martinel2018wide}. V tem odseku jih v grobem razdelimo na klasifikacijske in segmentacijske pristope~\cite{src:konstantakopoulos2023review, src:min2019survey}. Klasifikacijski pristopi v glavnem izvajajo enoznačno ali večznačno klasifikacijo jedi oziroma sestavin na sliki~\cite{src:martinel2018wide, src:tan2019efficientnet, src:bolanos2017food, src:fu2024recognizing}. Segmentacijski pristopi pa skušajo lokalizirati jed ali njeno komponento na sliki, ločiti posamezne komponente od preostalih ter jih potencialno tudi kategorizirati~\cite{src:wu2021large, src:bolanos2016simultaneous, src:poply2021refined, src:konstantakopoulos2023review, src:javadi2024smart}. Obstajajo tudi pristopi, ki zaradi izboljšanja rezultatov ali poenostavitve podatkovne zbirke kombinirajo metode obeh področij~\cite{src:zhu2023new}.
        ...
        \subsection{Klasifikacija jedi}
            ...
            S klasičnimi metodami značilke tipično izluščimo ročno, nato pa jih uporabimo v razvrščevalniku~\cite{src:min2019survey}.
            ...
            Nekateri avtorji so ocenjevali tudi druge modele iz te družine. V~\cite{src:vijayakumari2022food} so z EfficientNetB0 dosegli 80~\% natančnost. Izbrana različica uporablja zgolj 11 milijonov parametrov, kar omogoča izvajanje na precej širšem naboru strojne opreme.
            ...
            Bolanos~\etal se v delu~\cite{src:bolanos2017food} namesto klasifikacije samih jedi lotijo klasifikacije sestavin, prisotnih na sliki. Uporabili so zbirko Food-101~\cite{src:bossard2014food}, ki sicer vsebuje oznake jedi, a so jih ročno zamenjali z oznakami sestavin. Uporabili so standarden večznačen pristop razvrščanja z InceptionV3~\cite{src:szegedy2016rethinking} % večznačen - multi-label
            in ResNet50~\cite{src:he2016deep}ter dosegli do 80{,}1~\% natančnosti po metriki razvrščanja F1. Preverili so tudi sposobnost generalizacije modela na nove slike jedi, kjer so na modificirani zbirki Recipes5k~\cite{src:bolanos2017food} dosegli do 47~\% po metriki F1. Slabost njihovega pristopa je, da so vsakemu receptu pripisali množico sestavin, ne glede na to, ali je dana sestavina vidna ali ne.
        ...
        \subsection{Segmentacija jedi}
            ...
            Pouladzadeh~\etal~\cite{src:pouladzadeh2014using} so uporabili t.i. metodo reza v grafu (angl. graph cut)~\cite{src:boykov2001interactive-graphcut} za določanje regij, te pa so razvrstili z metodo SVM~\cite{src:cortes1995svm}.

            % Deep learning segmentation - intro
            Pristopi, ki uporabljajo globoko učenje, dosegajo bistveno boljše rezultate~\cite{src:minaee2021image}.
    ...
    \section{Izvajanje na robnih napravah} \label{sct:izvajanje-na-robnih-napravah}
        ...
        Mikrokrmilniki imajo pogosto na voljo do 16~MB Flash pomnilnika in do 8~MB RAM pomnilnika, kar je bistveno manj od običajne strojne opreme za izvajanje modelov globokega učenja~\cite{src:tinymlsurvey}.

        % Early efforts
        Za področje izvajanja modelov na robnih napravah se pogosto uporablja termin TinyML~\cite{src:abadade2023comprehensive}. Med začetke teh pristopov sodijo Kawano~~\etal~\cite{src:kawano2014foodcam} z modelom FoodCam-256, pripravljenim za izvajanje na mobilnih napravah, a uporabljajo tradicionalne pristope za klasifikacijo. Med pomembnejše pristope na tem področju sodita še SqueezeNet~\cite{src:iandola2016squeezenet} ter MobileNetV1~\cite{src:howard2017mobilenets}, ki v primerjavi z AlexNet~\cite{src:krizhevsky2012imagenet} ohranita oz. izboljšata natančnost, hkrati pa močno zmanjšata število parametrov.
        ...
        Omenjene rešitve tipično uporabljajo stroge optimizacije in omejen nabor operacij, s čimer skrajšajo čas inference in zmanjšajo potrebno količino pomnilnika~\cite{src:abadade2023comprehensive}. Za optimizirano izvajanje na ARM Cortex-M jedrih pogosto uporabljamo knjižnica CMSIS-NN~\cite{src:lai2018cmsis}.
        Za optimizirano izvanjanje modelov na STM32 mikrokrmilnikih pogosto uporabljamo izvajalno okolje STM32Cube.AI MCU, ki je del X-CUBE-AI, vmesnika v programskem okolju STM32CubeIDE~\cite{src:stxcubeai-docs}.
        ...
        % DSORT-MCU
        Problem združevanja objektov med rojenjem so deloma razrešili Boyle~\etal v DSORT-MCU~\cite{src:boyle2024dsort}, kjer so predstavili adaptivno deljenje slike. Sliko so najprej razdelili na prekrivajoče odseke, nato pa vsak odsek ločeno poslali skozi model FOMO~\cite{src:fomodocs}. Rezultate vseh so nato združili v končni izhod. S tem so izboljšali sposobnost modela FOMO za štetje objektov.
        ...
        % TODO: S tem bi moral začet sorodna dela robne naprave.
        ...
        Nikodinovska~\cite{src:Nikodinovska_2024} je na STM32 platformi implementirala model za klasifikacijo zapečenosti piškotov. 
        ...
        % TODO: Vsak model in zbirka rabi citat, vsaj v related work poglavju. Popravi za nazaj.
        % TODO: Piši v pretekliku

\chapter{Teoretično ozadje} \label{sct:teoreticno-ozadje}

% Sem spravi teorijo iz sorodnih del
...
\chapter{Metodologija} \label{sct:metodologija}
    ...
    \section{FOMO} \label{sct:fomo}
        ...
        \subsection{Arhitektura}
            ...
            \begin{itemize}
            \item \textbf{Prva plast}: konvolucijska plast z $32$ filtri, z aktivacijo ReLU~\cite{src:nair2010rectified-relu}, s prilagodljivo velikostjo jedra in koraka,
    ...
    \section{YOLOv11-seg} \label{sct:yolov11n-seg}
        ...
        Poleg funkcionalnih različic so modeli YOLOv11 na voljo v več velikostih: \enquote{nano}, \enquote{small}, \enquote{medium}, \enquote{large} in \enquote{extra large} (oznake \verb|n|, \verb|s|, \verb|m|, \verb|l|, \verb|x|).
    ...
    \section{Pretvorba semantično-segmentacijskih mask v okvirje} \label{sct:sam}
        ...
        Ustrezne podatkovne zbirke s sestavinami hrane, anotiranimi z okvirji (detekcijski kontekst), nismo zasledili. Razmišljali smo tudi o uporabi zbirke z anotacijami za \emph{instančno} segmentacijo (ločena maska za vsak objekt), vendar takšne zbirke prav tako nismo našli; če bi bila na voljo, bi maske lahko \enquote{naivno} pretvorili v oznake z okvirji~\cite{src:seychell2023efficient-mask2annotation} in jih uporabili za učenje detekcijskih modelov.
    ...
    \subsection{SAM} \label{sct:sam}
        ...
        SAM (angl. Segment Anything) model~\cite{src:kirillov2023segment} je \enquote{promptabilen}, razredno neodvisen segmentacijski okvir, ki iz preprostih namigov (točke, okvirji ali grobe maske) napove visokokakovostne maske. Arhitekturno združuje slikovni enkoder na osnovi ViT z lahkim dekoderjem za namige/maske, njegova bistvena prednost pa je t.i. \enquote{zero-shot} inferenca - segmentira lahko objekte, brez da bi poznal kategorijo izbranega objekta. 
