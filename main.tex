...
\chapter{Uvod} \label{sct:uvod}
    ...
    % Kratek opis problema. Justify the problem we are trying to solve, and justify the way we are trying to solve. 
...
%     - A je kdo kej v zvezi z dolocanjem jedi dajal na mikrokrmilnike
    ...
    \section{Razpoznavanje jedi na podlagi slik}
        ...
        \subsection{Klasifikacija jedi}
            ...
            % Upoštevanje informacije o sestavinah %TODO TO DEJ V PRVO OMEMBO BOSSARD2014
...
\chapter{Diskusija} \label{sct:diskusija}
    ...
    Na podlagi izkušenj predlagamo naslednje usmeritve:
    % DODAJ: 
    % 1) Preveri multi-label klasifikacijo prisotnosti sestavin kot cenejšo alternativo detekciji/segmentaciji na MCU.
    % 2) Prednauči hrbtenice na bolj relevantnih zbirkah (npr. Food-101 namesto ImageNet) in dodaj citat o koristnosti domeni-specifičnega predučenja.
    % 3) Razmisli o hibridih: FOMO kot nizkoločljivostna “attention” mapa + plitva klasifikacijska glava za recepte (end-to-end).
    % 4) Povečaj receptivno polje: patch-based pristop z MobileNetV2 na zaplatah, ali globlja hrbtenica ob enaki latenci.
    % 5) Uporabi tehnike šibko nadzorovanega učenja (attention med ekstraktorjem in klasifikatorjem) za učenje na receptih.
    % 6) Izboljšaj poenostavitev zbirke z logičnimi super-kategorijami; ocenjuj vpliv na metrike in kompleksnost.
    
    % (po želji) Dodaj sklepni odstavek Diskusije
    % DODAJ: 3–4 stavke, ki povežejo, kaj se je izkazalo za učinkovito, kaj ne, in kako to vodi v sklepno poglavje.

    % Pokaži konfuzijsko matriko za navadno učenje na poenostavljenem datasetu (Če ni prevelika), mogoče primerjaj načine uteževanja, komentiraj katere razrede dobro rešimo, katere meša med sabo in ugibaj zakaj jih meša med sabo.

    % Open world problem - makes it far more difficult
    
    % Kaj so problemi FOMO modela, in kaj drugi članki pravijo o njemu

    % Kako bi se še lotil reševanja tega problema na mikrokrmilniku, če bi vedel vse, kar veš zdaj? Kako bi se lotil tega, če bi še vedno uporabljal isti dataset?
        % Verjetno bi namesto nepreverjenih modelov brez člankov raje uporabil nekaj iz STM32 model zoo, ker imajo precejšen nabor
        % Verjetno bi naredil eksperimente z multi-label klasifikacijo, da se vidi, če je sploh treba tak komplicirat z detekcijskimi modeli.
        % Mogoče bi lahko nadaljeval s FOMO, vendar bi ga na naš način samo prednaučil, nato pa mu na konec dodal eno polnopovezano plast in ga učil direktno na receptih. S tem bi dobil end-to-end rešitev, ki bi morda celo delala.
        % Mogoče bi kako lahko povečal receptivno polje posamezne zaplate. Mogoče bi se lahko tega lotil z deljenjem slike na zaplate in vsako zaplato posebej poslal čez MobileNetV2 klasifikator.
        % Mogoče se bi splačalo uporabit kakšne end to end rešitve za predvidevanje receptov, ker tudi to obstaja. Mogoče rešitev, kjer samo pogledaš prisotnost sestavine, nato pa v tabelo za izbiro recepta, ni ustrezna.
        % Mogoče bi lahko vzel ideje iz weakly supervised pristopov in dodal nek attention layer med ekstraktor in klasifikator, in ga uporabili za učenje receptov. Naš pristop celo malo spominja na to, ampak smo ta hipotetičen attention layer (nizkoresolucijsko segmentacijsko masko) učili neposredno na FoodSeg103, klasifikatorja pa še nismo implementirali.
        % FOMO (oz. vbistvu katerikoli drug model bi uporabu) bi prednaučil na bolj relevantnem datasetu, kot recimo Food101. Načeloma sem do zdaj za osnovo uporabljal MobileNetV2, prednaučen na \textbf{ImageNet} podatkovni zbirki, ki ima le nekaj kategorij, povezanih s hrano. Sklepal bi, da bi bile uteži, prednaučene na Food-101, precej bolj ustrezne. (preglej katere kategorije v ImageNet imajo vezo s hrano, in citiraj nekaj, kar podpira trditev da relevanten dataset za predučenje pomaga pri končnih rezultatih).
        % Drug dataset? Preprostejša naloga, še posebej za tako omejene resource? Drugače je, če maš na voljo A100 al pa STM32 mikrokrmilnik.
        % Lahko bi ga mogoče poenostavili drugače - združili kategorije v logične "super-kategorije", ki bi jih določili na podlagi videza ter lastnosti sestavin. To bi verjetno zahtevalo precej dela.
    % Je to dejansko failure, ali je samo tak težek dataset? Zakaj je dataset tako težek? - mogoče to za zaključek?
        % Pomoje je deloma tudi failure, ker ima model očitne pomanjkljivosti, kot je majhno receptivno polje. Ravno tako strojna oprema in zahteve dopuščajo kompleksnejši model, morda z daljšim ekstraktorjem značilk.
        % Hkrati pa lahko na podlagi (tega magistrskega) dela veliko sklepamo o ustrezni poti za naprej. Problem razpoznave sestavin ni preprost, ravno tako še ne ustrezno rešen (citiraj ustrezne članke). Naš problem (angl. use-case) se načeloma razlikuje od tipičnih pristopov k reševanju zbirk kot je FoodSeg103, saj se tega tipično lotijo s segmentacijskimi modeli. To ni nujno ustrezno za našo končno uporabo, prav tako se načeloma ne ciljajo na izvajanje na mikrokrmilnikih (to doda precej kompleksnosti) (večina potencialnih rešitev razpoznave sestavin, npr ono z učenjem z recepti, odpade, ko dodamo to zahtevo).

    % SAM
    %   Tu imamo probleme, ker pri sestavinah hrane ni tako jasno določeno, kaj je objekt. % Opiši problem, citiraj Koporec2020
